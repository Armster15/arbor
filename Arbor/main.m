//
//  Original Source: https://github.com/beeware/briefcase-iOS-Xcode-template/blob/main/%7B%7B%20cookiecutter.format%20%7D%7D/%7B%7B%20cookiecutter.class_name%20%7D%7D/main.m
//  See for more info: https://github.com/beeware/Python-Apple-support/blob/main/USAGE.md
//
//  This file does have modifications made by me though.
//
//  main.m
...
static void add_site_dir(NSString *resourcePath, NSString *dir_name, PyObject *addsitedir) {
    ...
    NSLog(@"Adding %@ as site directory: %@", dir_name, path);

    if (path_str == NULL) {
        crash_dialog([NSString stringWithFormat:@"Could not decode %@ path", dir_name, nil]);
        exit(-13);
    }

    unicode_path = PyUnicode_FromWideChar(path_str, wcslen(path_str));
    if (unicode_path == NULL) {
        crash_dialog([NSString stringWithFormat:@"Could not convert %@ path to unicode", dir_name, nil]);
        exit(-13);
    }
    PyMem_RawFree(path_str);

    method_args = Py_BuildValue("(O)", unicode_path);
    if (method_args == NULL) {
        crash_dialog([NSString stringWithFormat:@"Could not create arguments for site.addsitedir for %@", dir_name, nil]);
        exit(-14);
    }

    result = PyObject_CallObject(addsitedir, method_args);
    if (result == NULL) {
        crash_dialog([NSString stringWithFormat:@"Could not add %@ directory using site.addsitedir", dir_name, nil]);
        exit(-15);
    }
}
...
int start_python_runtime(int argc, char *argv[]) {
    ...
    @autoreleasepool {
        ...
        if (app_module_name == NULL) {
            app_module_name = @"app.__main__";
            NSLog(@"MainModule not set in Info.plist; defaulting to %@", app_module_name);
        }
        ...
        @try {
            ...
            if (result == NULL) {
                ...
                NSLog(@"Python app module executed under Swift-managed app lifecycle.");
            }
        }
